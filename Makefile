APP=kube-serv
PROJECT=github.com/oleg-kalashnikov/kube-serv
...
KUBESERV_LOCAL_HOST?=0.0.0.0
KUBESERV_LOCAL_PORT?=8080
KUBESERV_LOG_LEVEL?=0

# Namespace: dev, prod, release, cte, username ...
NAMESPACE?=cte

# Infrastructure: dev, stable, test, minikube ...
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${KUBESERV_LOCAL_PORT}:${KUBESERV_LOCAL_PORT} \
		-e "KUBESERV_LOCAL_HOST=${KUBESERV_LOCAL_HOST}" \
		-e "KUBESERV_LOCAL_PORT=${KUBESERV_LOCAL_PORT}" \
		-e "KUBESERV_LOG_LEVEL=${KUBESERV_LOG_LEVEL}" \
